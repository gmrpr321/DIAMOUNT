    ...
    def decideImageType(self, title):
        Response = openai.ChatCompletion.create(
            messages=[
                {
                    "role": "system",
                    "content": """You are a helpful assistant that assists in returning a string by following these steps:
    ...
    def sendFlowchartAIRequest(self, title):
        Response = openai.ChatCompletion.create(
            messages=[
                {
                    "role": "system",
                    "content": """You are a helpful assistant that assists in returning an array that has following data:
                ...
                {
                    "role": "user",
                    "content": "Structure of a Programming language compiler",
                },
                {
                    "role": "assistant",
                    "content": "[Lexical Analysis,Syntax Analysis,Semantic Analysis,Intermediate Code Generaiton,Optimization,Object Code Generation]",
                },
                {"role": "user", "content": f"{title}"},
            ],
            model=aiModel,
    ...
    def sendComponentDiagramAIRequest(self, title):
        Response = openai.ChatCompletion.create(
            messages=[
                ...
                {
                    "role": "assistant",
                    "content": "[CAR,Engine,Transmission,Suspension,Brakes,Steering system,Exhaust system,Fuel system,Electrical system,Tires,Body]",
                },
                {"role": "user", "content": f"{title}"},
            ],
            model=aiModel,
    ...
    def sendPyramidChartAIRequest(self, title):
        Response = openai.ChatCompletion.create(
            messages=[
                ...
                {
                    "role": "assistant",
                    "content": "[Fats, Dairy, Protein, Fruits, Vegetables, Grains]",
                },
                {"role": "user", "content": f"{title}"},
            ],
            model=aiModel,
